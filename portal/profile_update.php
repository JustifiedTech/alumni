...
if (count($error)==0){
			...
			$update = mysqli_query($connect, "UPDATE users SET 
				`user_type`='$user_type',`first_name`='$Firstname', `other_name`='$Othername', `last_name`='$Lastname', `dob`='$dateofbirth', `email`='$email', `phone_number`='$phone', `gender`='$Gender', `faculty`=	'$faculty', `department`='$department', `course` ='$course', `qualification`='$qualify', `admin_year`='	$Entry', `grad_year`='$exit', `matric_no`='$matric', `prog_duration`='$duration', `address`='$address', `expertise`='$expertise' WHERE `users_id` = '$update_ID' ");
	...
	<div class="col-lg-3 col-sm-5 ">
		<div class="form-group">
			...
			<select name='faculty' class='form-control' required="yes" value="<?php echo $fa; ?>">
				...
				<select name="user_type" class='form-control' value="<?php echo $use; ?>">
				<option value="">--Please Select--</option> 
